...
<div class="content">
    ...
    <div class="panel panel-success" data-sortable-id="ui-widget-11" >
        <div class="panel-heading">
            <h4 class="panel-title">Simpanan Sukarela</h4>
        ...
        <div class="panel-body">
            ...
            <div class="btn-group" style="margin-bottom:2%">
                <button type="button" class="btn btn-green btn-sm" onclick="tambah_import()"><i class="fa fa-calendar-alt"></i> Import Data Simpanan Sukarela</button>
                <button type="button" class="btn btn-blue btn-sm" onclick="tambah()"><i class="fa fa-plus"></i> Buat Simpanan Sukarela</button>
            </div>
            @endif
            <table id="datafixedheader" class="table table-striped table-bordered table-td-valign-middle">
                ...
                <tbody>
                    ...
                    @foreach(get_simpanansukarela() as $no=>$data)
                        <tr class="odd gradeX">
                            <td>{{$no+1}}</td>
                            <td>{{$data->nik}}</td>
                            <td>{{$data->user['name']}}</td>
                            <td>{{uang(saldo_simpanansukarela($data->nik))}}</td>
                            <td>
			...
			<div class="modal-dialog modal-lg" style="margin-top:0px">
				<div class="modal-content">
                    <div class="modal-header">
						<h4 class="modal-title">Tambah Simpanan Sukarela</h4>
						<button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
					</div>
					<div class="modal-body">
                        <div class="alert alert-yellow fade show m-b-10" style="padding: 1%;text-align: center;">
                            <span class="close" data-dismiss="alert">×</span>
                            <strong>Notifikasi!</strong><br>Tambah data simpanan Sukarela anggota.
                        ...
                        <form id="mytransaksi" action="{{url('/Simpanansukarela')}}?_token={{csrf_token()}}" method="post" enctype="multipart/form-data">
			...
			<div class="modal-dialog modal-lg" style="margin-top:0px">
				<div class="modal-content">
					...
					<div class="modal-body">
                        ...
                        <form id="myimport" action="{{url('Simpanansukarela/import')}}" method="post" enctype="multipart/form-data">
                            <div class="form-group row m-b-10">
...
@push('ajax')
    <script>
        ...
        function view_tagihan(nik){
			...
			$.ajax({
				type: 'GET',
				url: "{{url('Simpanansukarela/view_tagihan')}}",
				data: "nik="+nik,
				success: function(msg){
        ...
        function simpan(){
            ...
            var token= "{{csrf_token()}}";
                $.ajax({
                    type: 'POST',
                    url: "{{url('/Simpanansukarela')}}?_token={{csrf_token()}}",
                    data: new FormData(form),
                    contentType: false,
                    cache: false,
                    processData:false,
        ...
        function proses_import_data(){
            ...
            var token= "{{csrf_token()}}";
                $.ajax({
                    type: 'POST',
                    url: "{{url('/Simpanansukarela/import')}}?_token="+token,
                    data: new FormData(form),
                    contentType: false,
                    cache: false,
                    processData:false,
